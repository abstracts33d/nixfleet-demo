...
#
# Cache distribution is handled separately by `modules/demo-cache.nix`
# (harmonia serving /nix/store on :5000), not by this coordinator bundle.
{lib, ...}: {
  options.nixfleet.coordinator = {
    enable = lib.mkEnableOption ''
      This host is a fleet coordinator. Sets forge / ci-runner /
      release-signer enable flags to lib.mkDefault true and acts as a
      discovery flag for scopes that want to adjust behaviour when
      running on a coordinator (for instance, the backup client scope
      skipping itself).
    '';
