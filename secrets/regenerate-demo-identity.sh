...
# Hosts that run a nixfleet-agent. forge is intentionally NOT in this
# list -- it's the CI/cache/trust anchor outside the fleet. Including
# forge as an agent target creates a chicken-and-egg between its
# build-time-baked trust pin and the release key it regenerates on
# every install.
agent_hosts=(cp web-01 web-02)

# Token validity: 10 years. The bootstrap tokens are committed to the
# demo repo as PUBLIC material so fresh clones can enroll without an
# extra regenerator step. The "long-lived credential" caveat doesn't
# apply here - these tokens are intentionally public, no security
# property hinges on their lifetime. 10y > expected demo shelf life.
token_validity_hours=87600
...
if [ -f secrets/age-identity.txt ] && [ "$force" = 0 ]; then
  ...
  if command -v age-keygen >/dev/null 2>&1; then
    # age-keygen -o refuses to overwrite an existing file. Remove
    # any prior identity first so --force is actually forceful.
    rm -f secrets/age-identity.txt
...
# --- Step 7b: emit secrets/bootstrap-nonces.nix from tokens ----------------
# v0.2 closes the replay-after-DB-wipe vector (nixfleet#96) by requiring
# the agent's enrollment nonce to appear in a signed allowlist. CI signs
# this list as releases/bootstrap-nonces.json; CP polls and refuses any
# /v1/enroll whose nonce isn't present.
#
# The nonce values are public-by-design (the bootstrap-token itself is
# the secret -- signed by the org root). Publishing the nonce list to
# git is exactly what CP needs.
nonces_file="secrets/bootstrap-nonces.nix"
{
  echo "# Generated by secrets/regenerate-demo-identity.sh -- DO NOT HAND-EDIT."
  echo "#"
  echo "# Operator-declared allowlist of valid bootstrap-token nonces."
  echo "# Each entry mirrors the corresponding token's claims."
  echo "# CI passes this to nixfleet-release via --bootstrap-nonces-attr."
  echo "["
  for host in "${agent_hosts[@]}"; do
    token="secrets/bootstrap-tokens/$host.json"
    [ -f "$token" ] || continue
    nonce=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["claims"]["nonce"])' "$token")
    hostname=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["claims"]["hostname"])' "$token")
    expires=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["claims"]["expiresAt"])' "$token")
    issued=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["claims"]["issuedAt"])' "$token")
    echo "  {"
    echo "    nonce = \"$nonce\";"
    echo "    hostname = \"$hostname\";"
    echo "    expiresAt = \"$expires\";"
    echo "    mintedAt = \"$issued\";"
    echo '    mintedBy = "regenerate-demo-identity.sh";'
    echo "  }"
  done
  echo "]"
} >"$nonces_file"
chmod 0644 "$nonces_file"
...
# --- Step 8b: cache signing key for forge's harmonia binary cache ----------
# forge runs harmonia on :5000 (modules/demo-cache.nix). It signs closures
# served from forge's /nix/store with the private half below; agent hosts
# trust the matching pubkey (baked into closures via
# modules/use-forge-cache.nix). Without this, agents on cp/web hosts
# can't realise closures CI built and rollouts stall.
if [ -f secrets/cache-signing-key ] && [ "$force" = 0 ]; then
  echo "secrets/cache-signing-key already exists. Pass --force to regenerate."
else
  rm -f secrets/cache-signing-key secrets/cache-signing-key.pub
  nix-store --generate-binary-cache-key \
    "nixfleet-demo-1" \
    secrets/cache-signing-key \
    secrets/cache-signing-key.pub
  chmod 0644 secrets/cache-signing-key.pub
  chmod 0600 secrets/cache-signing-key
fi
...
echo "  - secrets/bootstrap-nonces.nix           (allowlist mirrored into fleet.bootstrapNonces; CI signs this)"
echo "  - secrets/cache-signing-key{,.pub}       (harmonia signs closures with the private half; agents trust the pub)"
